...
Keep the tutorial handoff brief: checkpoint/baseline, observed evidence, open question/decision, and next action. Do not create a generation/writer protocol or a second phase plan. If interrupted before synchronization, reread source/diffs on resume and check affected evidence. Current documentation explains only the current baseline/target and evidence; remove prose about previous editing mistakes, reviewer fixes, or document revision comparisons. Preserve valid design rationale instead of retaining incorrect reasoning followed by a correction. State invariants/decisions where used, and update affected restatements when contracts change.
