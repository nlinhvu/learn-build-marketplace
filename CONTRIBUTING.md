...
Learn OSS lives in `plugins/learn-oss-vn` with `learn-catalogue` and `learn-recipe`. Preserve its distinct priority: accessible learning, useful community sharing, then contribution. Keep glossary, rendered diagrams, numbered walkthroughs and code consistent; separate public recipes from personal learning records. Catalogue coverage and source evidence must stay honest. Shared references/assets remain identical within its two self-contained skills.
